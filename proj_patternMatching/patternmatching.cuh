...
template <typename T>
__device__ T reduce_sum(thread_group g, T *temp, T val)
{
...
__global__ void naivePatternMatchKernel_LS(
  const T* d_inputs, // numInputs * length
  ...
  const T* b_input = d_input[iIdx * length];
