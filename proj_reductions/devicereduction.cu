...
int main(int argc, char **argv) {
  ...
  const size_t sectionLength = 10;
  const size_t numSections = 6;
  thrust::device_vector<AccumulatorType> d_sumSections(numSections, 0);
  thrust::device_vector<AccumulatorType> d_sumSqSections(numSections, 0);
  ...
  for (size_t i = 0; i < numSections; ++i) {
    for (size_t j = 0; j < sectionLength; ++j) {
      ...
      printf("Section %d index %d = %hu\n", i, j, h_x[i * sectionLength + j]);
  ...
  thrust::device_vector<unsigned int> d_validSectionSizes(numSections, 0);

  // Call sectioned kernel
  NUM_BLKS = d_x.size() / TPB + 1;
  printf("Total threads in grid = %zd\n", (size_t)NUM_BLKS * (size_t)TPB);
  device_sectioned_sum_and_sumSq_kernel<BaseType, AccumulatorType, unsigned int>
