...
int main() {
  ...
  cub::DeviceRunLengthEncode::Encode(nullptr, temp_storage_bytes, d_keys.data().get(),
  ...
  for (int iter = 0; iter < 3; ++iter){
    cub::DeviceRunLengthEncode::Encode(d_temp_storage.data().get(), temp_storage_bytes, d_keys.data().get(),
                                       d_unique_out.data().get(), d_counts_out.data().get(),
                                       d_num_runs_out.data().get(), d_keys.size());
  }
