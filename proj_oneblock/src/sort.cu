...
__global__ void OneBlockSortKeys(
  const int *d_keys_in,
  int *d_keys_out,
  const int num_items
){
...
int main() {
  ...
  for (int i = 5; i <= 20; i++) {
    ...
    auto block_sort_temp_bytes = sizeof(typename cub::BlockRadixSort<KeyT, NUM_THREADS, ITEMS_PER_THREAD>::TempStorage);
    auto block_exchg_temp_bytes = sizeof(typename cub::BlockExchange<KeyT, NUM_THREADS, ITEMS_PER_THREAD>::TempStorage);
    auto smem_size = (std::max) (block_sort_temp_bytes, block_exchg_temp_bytes);
    printf("smem_size: %zd\n", smem_size);
    OneBlockSortKeys<KeyT, NUM_THREADS, ITEMS_PER_THREAD>
        <<<1, NUM_THREADS, smem_size>>>(d_input.data().get(), d_output.data().get(),
                             (int)num_items);
